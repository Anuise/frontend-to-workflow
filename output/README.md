...
output/
  my-admin-ui/
    ...
    workflow.drawio             # f2w-diagram（分支）   交付物：Navigation diagram（draw.io）
...
| 分支（與步 4 併行） | `f2w-diagram` | `workflow.json` | `workflow.drawio` |
...
- **交付物**：`workflow.xlsx` 不預期手改，內容不對回頭改上游 json 再重跑 `f2w-export`。`workflow.drawio` 同理（重跑直接覆寫）：可在 draw.io 圖形化調版面，但要保留手調排版請自行另存副本。
