...
- [x] `f2w-revise workitems <修改需求>` 落檔並列出每一筆全文，行為與 T2 的 workflow 側一致
- [x] 缺 `workitems.json` 時丟 `MissingPrerequisiteError`，提示先跑 `f2w-breakdown`，中止
- [x] 重跑 `f2w-breakdown`，`set` 過的工項欄位、`upsert` 補的工項、`remove` 刪的工項三者全部照舊生效
- [x] `upsert` 的兩個分支都蓋到：id 已存在則整筆覆蓋、不存在則新增
- [x] 自訂 id（如 `BE-EXTRA-01`）的 `upsert` 工項在重跑後仍在
- [x] `upsert` 與 `remove` 共用作用點：`remove` 後 `upsert` 同 id 則工項活著，反之被刪
- [x] 固定序 `remove → upsert → set`，`set` 作用在最終存在的物件上
- [x] `set` 覆蓋純量欄位與覆蓋整組 `dependsOn` 陣列都可行
- [x] `inferred` 不在可覆蓋欄位集內，契約層擋下
- [x] 套用後跌破前端工項顆粒度底線時整步丟錯、**不落地**
- [x] 套用後仍跑既有五重把關（涵蓋、參照、顆粒度全部照舊）
- [x] 孤兒工項修訂保留該筆、發 warning、其餘照套
- [x] `buildWorkitems` 的薄接線案例：只證明傳入的修訂確實被套進輸出，深層行為不在這裡重複
- [x] 改寫 `f2w-breakdown` 的 SKILL.md：多一個前置輸入、存檔前套用、欄位凍結語意
- [x] `f2w-revise` 的 SKILL.md 補上 workitems 段，含「後端 id 可能漂、建議補漏用自訂 id」這個實務建議
