...
3. **列舉 Page**（從 `baseUrl` 廣度優先走訪，用瀏覽器工具）
   ...
   - **狀態式 Page 必須截到真實狀態**：同一 route 下只要 `tab` 名稱不同，截圖前就必須先把 UI 實際操作到該狀態；不可用同一張模組首頁截圖代替多個 tab／modal／drawer／wizard Page。每個 Page 的截圖必須由瀏覽器在該狀態下直接產生，不可用複製既有 PNG 的方式補截圖。保存前抽查 `pages.json` 的截圖檔：若相鄰或同模組多個 Page 的 PNG 位元組完全相同，除非它們真的代表同一個 Page，否則視為 capture 未完成，必須回頭補按控制項重截。
   - **不可達內部狀態不可列 Page**：若程式碼裡存在 modal／drawer／state，但目前 UI 沒有任何可見、可操作控制項能進入該狀態，不可用直接改 React state、執行 console script、或複製其他截圖的方式硬補；應從 `pages.json` 移除或標註為不可達，直到前端補上實際入口。
   - **模組內按鈕掃描**：進入頁籤式模組後，要逐一掃描目前畫面可見的主要按鈕與表格列操作（例如「查看」「審核」「編輯」「新增」「管理報表」）。安全按鈕要實際點開並截下開啟後狀態；會立即寫入或破壞資料的按鈕只記錄為 action，不確認、不執行。
...
自動截圖完成後，不要只看檔案是否存在；在交給 `f2w-describe` 前，先整理 `pages.json` 與 `screenshots/`，讓它們能代表完整、可讀的使用者流程。

1. **確認重要畫面完整性**
   - 檢查主要頁面、tab、modal、drawer、wizard step、表單往下捲動後的內容、危險操作確認彈窗、設定變更彈窗是否都有截到。
   - 對可捲動的 modal／面板，若首屏看不到關鍵欄位或底部操作，補截「往下」狀態並列為獨立 Page。
   - 登入、角色切換、空狀態、錯誤狀態若是實際使用流程的一部分，也應列入。

2. **依使用者流程或功能模組排序**
   - 把同一主題的畫面排在一起，避免相關截圖散落在清單不同位置。
   - 優先用「入口 → 列表 → 詳情 → 編輯/新增/確認彈窗 → 子分頁」的順序；跨模組時依主要導覽或使用者工作流排序。
   - 重新排序後要同步更新 `pages.json` 中每筆 `screenshot` 的檔名對應。

3. **補齊缺漏互動狀態**
   - 若發現某個 action 會打開新狀態（例如新增、編輯、變更分配、刪除確認、查看案件），但沒有對應截圖，回到瀏覽器實際點開後補截。
   - 補截圖時要用真實 UI 操作到該狀態，不要複製既有 PNG 或直接捏造檔案。
   - 會寫入或破壞資料的操作只截確認前狀態；不可按最終確認。

4. **移除或確認重複畫面**
   - 比對相近檔名、相同尺寸、相同模組下的截圖；完全相同 hash 通常代表重複或漏操作。
   - 視覺上相近但代表不同流程狀態的畫面可以保留，但 `tab` 應命名清楚（例如「提交申請」與「編輯退回申請」）。
   - 真正重複時，刪除多餘 PNG、移除 `pages.json` 對應項目，並視需要重新編號。

5. **同步索引與檔名**
   - 任何新增、刪除、改名、重排都必須同步 `pages.json`，確保每個 `screenshot` 指向存在的檔案。
   - 檔名編號只是閱讀順序，不是 Page 識別；Page 識別仍以 `route` + `tab` 為準。
   - `tab` 名稱要表達狀態層級，必要時使用 `｜` 串接，例如 `專案詳情｜建立新任務｜往下`。

6. **最後驗證**
   - 確認 `pages.json` 筆數與 `screenshots/` 中預期 PNG 數量一致。
   - 確認 `pages.json` 的每個 `screenshot` 都存在，且沒有未被引用的暫存截圖。
   - 抽查新增或調整過的截圖，確認不是空白頁、錯誤頁、錯誤登入狀態或錯誤彈窗。
