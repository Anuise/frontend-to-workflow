...
2. **逐 Page 劃分前端工項**（以 `workflow.json` 為據、**繁體中文**）
   - 對 `workflow.pages` 列出的**每一個** Page，至少產出一筆前端 Work item（涵蓋不得漏頁），每筆填**內容型欄位**：
     - **id**：全域唯一的工項識別（跨前端＋後端）。
     ...
     - **risk**：風險備註（可空字串）。
3. **推論後端工項**
   - 從各 Page 的操作與內容推論所需的後端工作（如儲存 API、驗證、清單查詢），比照前端填同一組內容型欄位；這些工項一律是**推論·待確認**（`inferred` 由落在 backend 陣列決定，不用手填）。
4. **組裝並驗證** — `buildWorkitems(workflow, frontendItems, backendItems)`
   - 五重把關：**涵蓋**（每個 Page 至少一筆前端工項）、**參照**（每筆 `sourcePage` 存在於 `workflow.pages`、`dependsOn` 每個 id 存在於本批）、**id 全域唯一**、**inferred 旗標**（前端 false、後端 true，由陣列決定）。`sourcePage` 一律取自 `workflow.pages`（單一真實來源）。
...
`src/breakdown/`：`loadWorkflowForBreakdown`（前置檢查＋讀回 workflow.json）、`buildWorkitems`（涵蓋＋參照＋契約五重把關、由陣列決定 inferred 的組裝核心）、`saveWorkitems`（驗證後保存）。契約見 `src/contracts/workitems.ts`；路徑見 `src/output.ts`（`contractPath`）。決策見 ADR-0001（步驟間以檔案交接、checkpoint）與 ADR-0002（後端工項為 AI 推論）。
