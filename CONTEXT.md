...
以業務 Step 為節點、步驟之間的業務轉場為有向邊的交付圖 `mainflow.drawio`；讀者是**業主**。由 `f2w-diagram` 從 Workflow description ＋ Main flow 組出（`buildDiagram(workflow, mainflow)`）：一條主線一個 draw.io 分頁（第 1 頁就是第一條主線），**最後一頁固定是總覽**（分頁名「總覽」＝`OVERVIEW_PAGE_NAME`）——把每條主線那一列照 `flows` 順序由上到下排在同一頁，內容與該主線自己那頁逐字相同、只換 y 座標，是簡報與列印用的全景，不是另一種圖。節點是業務動作而不是 Page。圖上不標「推論·待確認」也不標 ⚠——要乾淨好懂，推論的表態留在 `mainflow.json` 與對話回報。含業務決策條件（見 Outcome 與決策點），但不含角色泳道與訊息事件。
_Avoid_: BPMN 圖, flowchart, 流程圖（泛稱）, 網站地圖, Navigation diagram（已被取代的舊語）

**Main flow（主線）**:
一個**頂層 Page 分類**（此類專案即側欄模組）收成的一條流程；是 `mainflow.json` 的 `flows[]` 一筆，也是 draw.io 的一個分頁單位。`name` **照抄該分類名、不改字**，`flows` 順序照 `workflow.json` 的頁序——業主看圖就是看側欄。邊界不推論，推論的是每條主線切成幾步、每步怎麼措辭（素材是每頁的頁面用途，Overview 只當語氣參考）。每條主線 1–20 個 Step、單列橫排不折行，各自一個色系（9 色色表依 `flows` 順序取、超過循環）；步數不必每頁一樣長，頁多的分類切細、只有一頁的分類就是一步。主線**不是單純的直鏈**——一步可以有多條 Outcome 而長出分歧，也可以指回前面的步而形成迴圈；但分歧與迴圈都只在**同一條主線之內**成立，跨主線的分歧畫不出來（一條主線一張分頁，跨分頁的邊沒有安放處），遇到時那個岔路就在圖上消失。推論落在交接檔 `mainflow.json`：**可手改，已存在就沿用、不重推論**（要重推得先刪檔或明講）。有涵蓋完整性硬驗——`steps[].pages` ∪ `excludedPages` 必須剛好等於 `workflow.json` 的頁集合、每頁只出現一次；不在任何主線上的落選頁圖上完全不提，只在 `excludedPages`（含一句 `reason`）與對話 warning 交代，完整操作清單仍以 `workflow-<時戳>.xlsx` 為權威；頂層分類全部上圖時 `excludedPages` 就是空陣列。
_Avoid_: Section（已退場的舊分頁單位）, Route（專指 `pages[].route` 的 URL 路徑）, 模組（產品功能區的泛稱；主線邊界雖與它一致，文件上一律稱主線）

**Step（步驟）**:
主線上的一個業務層級動作，是圖上一個節點：粗體「編號. 業務動作名」＋ 小字說明的兩段式。**一步可收攏多個 Page**（例如服務維護的五個 tab 收成「維運模型服務」一步）；只留主幹，彈窗與明細頁不畫，收攏的頁不上圖面、只進 tooltip（抬頭「此步驟涵蓋的頁面：」）。字數上限寫進契約由驗證擋下：標題 `title` ≤12 字、小字 `note` ≤30 字。一步往外走的去向**不再是一句 `edgeLabel`**（已退場的舊語，它一步只掛得住一條往下一步的邊），改成每步一組 Outcome——**每一步都要表態**，終點步明寫空陣列，單出口就是只有一條的那組。Outcome 的條件措辭是 AI 寫的業務判斷語，但每一條都要有 Evidence 墊背，撐不住就丟 `DiagramConsistencyError` 要求改順序或重新分步——不畫虛線。
_Avoid_: Page（Step 比 Page 大，一步可收攏多頁）, 節點（泛稱）, 任務（撞 Work item）, edgeLabel（已退場的舊語）

**Outcome（出口）**:
一步走得出去的一條邊：帶一句條件措辭（≤8 字的業務判斷語）、一個目標步號（限**同一條主線**、1 起算，可指回前面的步而形成迴圈，不得指向自己），以及一筆 Evidence。每步掛一組，**終點步明寫空陣列**——每一步都得對「還走不走得出去」表態，沉默不算答案；單出口就是只有一條的那組，有 ≥2 條時圖上長出一個決策點。
_Avoid_: edgeLabel（已退場的舊語）, transition, 分支（泛稱）

**Evidence（事實憑據）**:
一條 Outcome 所引用的那個畫面操作，以「路由 ＋（可選）tab ＋ 操作名」指認：那一頁必須屬於**來源步自己收攏的頁**，操作名必須**逐字**命中 `workflow.json` 上那頁 `actions[]` 的一筆；該操作若有操作去向，去向那頁必須落在目標步收攏的頁裡。它是讓「每條邊都有事實支撐」在**操作去向為 null 時依然成立**的方式——純前端專案的寫入型操作（核准、駁回、送出、刪除）去向一律是 null，最該畫成分歧的操作恰好一條去向都沒有，於是改以「這個操作確實存在於那一頁」當憑據。
_Avoid_: 佐證, 來源, reference（泛稱）

**決策點（Decision point）**:
一步有 ≥2 條 Outcome 時，圖上在該步之後長出的菱形節點：與該主線同色系、本身不寫字，**不佔步號、也不寫進 `mainflow.json`**——它是畫出來的分歧記號，不是推論出來的一步。該步先連一條無字的邊到菱形，菱形再往各個目標步各拉一條帶條件措辭的邊。
_Avoid_: Step（決策點沒有編號，不是一步）, gateway（BPMN 語彙）, 判斷節點（泛稱）
