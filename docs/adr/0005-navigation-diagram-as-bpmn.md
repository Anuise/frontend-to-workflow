...
> **其中「圖的主體是導覽」與「零推論」兩條已由 [ADR-0008](0008-main-flow-diagram-ai-inferred.md) 取代**（圖改成業主向的 Main flow diagram `mainflow.drawio`、主線推論落在 `mainflow.json` 交接檔）；三條底線只剩「確定性可被斷言」仍然有效。
