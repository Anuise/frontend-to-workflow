...
| 分支（與步 4 併行） | `f2w-bpmn` | `workflow.json` | `output/<project>/workflow.bpmn` |
...
- **f2w-bpmn**（分支步，與 f2w-export 併行）：讀 `workflow.json`，把每個 **Page** 映成 `userTask`（id 由正規化 route(+tab) 衍生、name 為頁面用途）、每個換頁操作映成 `sequenceFlow`（一頁有 ≥2 條出口時插一個 `exclusiveGateway`），不換頁的操作寫進該 task 的 `documentation`；`pages[0]` 接 `startEvent`、無出口的 Page 各接一個 `endEvent`。確定性算出 DI 座標（從入口 BFS 分層）後產出 **Navigation diagram** `workflow.bpmn`，可用 bpmn.io／Camunda Modeler／draw.io 開啟。**零推論**，不引入新的待確認維度；全圖無終點（純循環）與孤立頁會照實回報。決策見 ADR-0005。
...
`manifest.yml`、`pages.json`、`workflow.json`、`workitems.json` 都是**宣告式逃生口**：自動偵測／列舉／描述／拆項不完美時，可直接手改該檔再重跑後續步驟，重跑會尊重手改值。`workflow.xlsx`、`workflow.bpmn` 與 `workitems.xlsx` 是交付物、不是交接檔：`workflow.xlsx` 不預期手改，內容不對時回頭改上游的宣告式檔案再重跑 f2w-export；`workflow.bpmn` 同理（重跑直接覆寫，可在 Modeler 圖形化調版面但要另存副本才留得住）；`workitems.xlsx` 是畫押欄留白的**範本**、可被重跑覆蓋，人須另存一份**工作副本**填權責畫押值，重跑只覆蓋範本、不動工作副本。

## 各步細節

各步的流程、契約驗證與逃生口見該 step skill 本身：`f2w-start`、`f2w-capture`、`f2w-describe`、`f2w-export`、`f2w-bpmn`、`f2w-breakdown`、`f2w-breakdown-export`。詞彙定義見 `CONTEXT.md`。
