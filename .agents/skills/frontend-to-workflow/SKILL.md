...
- **f2w-diagram**（分支步，與 f2w-export 併行）：**推論步**。讀 `workflow.json`，以其 **Overview** 與每個 Page 的頁面用途為素材，推論出幾條 **Main flow（主線）**，先寫成可手改的交接檔 `mainflow.json`——**已存在就沿用、不重推論**（要重推請先刪檔或明講「重推主線」）；再依 `mainflow.json` 確定性地畫出多分頁的 **Main flow diagram** `mainflow.drawio`，讀者是**業主**：**一條主線一張大分頁、沒有總覽頁**（第 1 頁就是第一條主線），頁內是主線名 24px 粗體標題 ＋ 同色系細橫線 ＋ **單列橫排、不折行**的 2–7 個 **Step（步驟）**，每步兩段式（粗體「編號. 業務動作名」＋ 小字說明），每條主線各一個色系（固定色表依主線順序取、超過循環）。圖上**只畫主幹**，彈窗與明細頁不畫；**一步可收攏多個 Page**（例如服務維護的五個 tab 收成「維運模型服務」一步），收攏的頁不上圖面、只進 tooltip（抬頭「此步驟涵蓋的頁面：」）。邊的 label 是 AI 寫的業務轉場動作，但相鄰兩步之間必須有 `workflow.json` 裡真實的操作去向墊背（來源步任一頁 → 目標步任一頁），接不上就丟 `DiagramConsistencyError`、要求改順序或重新分步，不畫虛線；`steps[].pages` ∪ `excludedPages` 也必須剛好等於 `workflow.json` 的頁集合、每頁只表態一次。落選頁圖上完全不提，只在 `mainflow.json` 的 `excludedPages`（各附一句原因）與對話回報裡交代，完整操作清單仍以 `workflow.xlsx` 為權威。圖上**不標「推論·待確認」、不標 ⚠**——給業主看的圖要乾淨好懂。決策見 ADR-0006 與 ADR-0008。
